...
* Create AssumeRole, add the following block to a new file called security.tf resource "aws_iam_role" "ec2_instance_role" {
...
* }
*  
* Create IAM Policy resource "aws_iam_policy" "policy" {
...
* }
*  
* Attach the policy to the IAM Role resource "aws_iam_role_policy_attachment" "test-attach" {
*   role       = aws_iam_role.ec2_instance_role.name
*   policy_arn = aws_iam_policy.policy.arn
* }
*  
* Create an Instance Profile for the IAM Role resource "aws_iam_instance_profile" "ip" {
...
* Create a security group for the ALB in the security_groups.tf file: resource "aws_security_group" "alb-sg" {
*   name   = "alb-sg"
*   vpc_id = aws_vpc.main.id
*   ingress {
...
* }
*  
* Create bastion ALB resource "aws_lb" "bastion-alb" {
*   name     = "bastion-alb"
*   internal = false
*   security_groups = [
...
* }
*  
* Create a target group resource "aws_lb_target_group" "bastion-target-group" {
*   health_check {
*     interval            = 10
*     path                = "/"
*     protocol            = "HTTP"
...
* }
*  
* Createa a Listener for the target group resource "aws_lb_listener" "my-test-alb-listner" {
...
* }
*  
* Add the following lines to output.tf file: output "alb_dns_name" {
...
* }
*  
* Create a security group for the bastion instances to use in your launch template resource "aws_security_group" "bastion_sg" {
*   name = "vpc_web_sg"
...
* }
*  
* Create bastion launch template resource "aws_launch_template" "BastionLT" {
*   name = "BastionLT"
...
* Create the ASG itself resource "aws_autoscaling_group" "Bastion_asg" {
*   vpc_zone_identifier  = [
*     aws_subnet.public[0].id,
*     aws_subnet.public[1].id
*   ]
...
* Add the following block to keypair.tf file: resource "aws_key_pair" "Dare" {
*   key_name   = "Dare"
*   public_key = file("/path/to/id_rsa.pub")
* }
*  
* Add EC2 configuration to ec2.tf: resource "aws_instance" "bastion" {
*   count  = var.preferred_number_of_public_subnets == null ? length(data.aws_availability_zones.available.names) : var.preferred_number_of_public_subnets   
*   key_name      = aws_key_pair.Dare.key_name
*   ami           = var.bastion_ami
*   instance_type = "t2.micro"
...
* Create internal facing lb for your webservers: resource "aws_lb" "tooling-alb" {
*   name     = "tooling"
*   internal = true
*   security_groups = [
...
* Create a KMS Key for EFS, add the following to kms.tf resource "aws_kms_key" "kms" {
*   description = "KMS key"
*   policy = jsonencode({
*   "Version": "2012-10-17",
*   "Id": "kms-key-policy",
...
* }
*  
* Create EFS security group in security_groups.tf file resource "aws_security_group" "EFS-SG" {
*   vpc_id      = aws_vpc.main.id
*   name        = "EFS-SG"
...
* }
*  
* Create the EFS itself resource "aws_efs_file_system" "efs" {
*   tags =  {
*     Name = "efs"
*   }
*   encrypted = true
...
* }
*  
* Create mount points, one per AZ resource "aws_efs_mount_target" "mounta" {
